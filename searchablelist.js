...
class SearchableList extends Component {
    ...
    handleCheck = (event) => {
        ...
        this.state.filteredListDetails.map((item)=>{
            if(item.listIndex == event.target.dataset.id) {
                item.checked = !item.checked;

                if (item.checked) {
                    valueStr = (valueStr == '') ? item.listCode : (valueStr + ', ' + item.listCode);
                    optStr = (optStr == '') ? item.listOption : (optStr + ', ' + item.listOption);
                }
            }
        })
        var req = {
        ...
        this.props.inputJSON.fieldOption = optStr;
  ...
  render() {
    ...
    return (
          ...
          <MDBCol primaryId={td.primaryId}>
              <fieldset id={this.state.customId} data-component="SEARCHABLELIST" className={"searchableList font-14 compSearchList"+this.state.customClass+''+addStyles} style={mystyle}>
            <input
